#Programa que te permite buscar un color HSV a Buscar
import cv2
import numpy as np
...
cv2.createTrackbar('Hue Minimo', 'image', 0, 360, nothing)
cv2.createTrackbar('Hue Maximo', 'image', 0, 360, nothing)
cv2.createTrackbar('Saturation Minimo',' image', 0, 360, nothing)
cv2.createTrackbar('Saturation Maximo', 'image', 0, 360, nothing)
cv2.createTrackbar('Value Minimo', 'image', 0, 360, nothing)
cv2.createTrackbar('Value Maximo', 'image', 0, 360, nothing)
...
while(1): #While True
  ...
  if k == 27:
    print ("HSVmin=: ",[hMin,sMin,vMin],"HSVmax",[hMax,sMax,vMax])
    break
